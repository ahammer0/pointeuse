def webserver(db):
    ...
    def getStatus():
        if db.isActivePeriode:
            return f"""<p hx-get="/getStatus" hx-trigger="every 1s" hx-target="#state">Actif depuis: {db.currentPeriode.getStartTimeStr()}</p>"""+f"""<p>Duree: {db.currentPeriode.getDuration()}</p>"""
        else:
            return """<p>Inactif</p>"""
